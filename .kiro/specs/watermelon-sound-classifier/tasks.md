...
- [ ] 3. 데이터 증강 모듈 구현
  - [ ] 3.1 SNR 제어를 통한 소음 증강 함수 생성
    - 깨끗한 오디오를 소음 파일과 혼합하는 augment_noise() 함수 작성
    - SNR 계산 및 소음 스케일링 알고리즘 구현
    - 다양한 소음 유형과 SNR 레벨 지원 추가
    - 증강된 샘플을 위한 파일 명명 규칙 생성
    - _요구사항: 2.1, 2.2, 2.3, 2.4_

  - [ ] 3.2 증강 배치 처리 및 검증 추가
    - 여러 원본 파일에 대한 배치 처리 구현
    - 소음 파일이 없는 경우 원본 데이터만 사용하는 대체 로직 추가
    - 사용 가능한 소음 파일 수에 따른 동적 증강 배수 조정 구현
    - 증강된 샘플에 대한 오디오 품질 검증 추가
    - 임시 증강 파일에 대한 정리 기능 생성
    - SNR 계산 및 오디오 혼합에 대한 단위 테스트 작성
